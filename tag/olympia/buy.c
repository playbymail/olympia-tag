...
static int
seller_comp(const void *a_ptr, const void *b_ptr)
{
	trades_list a = (trades_list) a_ptr;
	trades_list b = (trades_list) b_ptr;
	if ((*a)->cost == (*b)->cost)
...
/* BUGFIX (modernization): updated parameters to const void * for qsort */
static int
buyer_comp(const void *a_ptr, const void *b_ptr)
{
	trades_list a = (trades_list) a_ptr;
	trades_list b = (trades_list) b_ptr;
	if ((*a)->cost == (*b)->cost)
 ...
 */
	 ...
	 */
	if (trades_len(l) > 0)
		qsort(l, (unsigned) trades_len(l), sizeof(struct trade *), buyer_comp);
#endif NEW_TRADE
...
resolve_trades_here(int where)
{
   ...
   */
  cur_buyer = 0;
  while (cur_buyer < trades_len(buyers)) {
    /*
     ...
     */
    for(cur_seller = 0;
	cur_seller < trades_len(sellers) && buyers[cur_buyer]->qty > 0;
	cur_seller++)
	...
	for(next_buyer = cur_buyer+1;
	    next_buyer < trades_len(buyers); next_buyer++)
...
delete_city_trade(int where, int item)
{
  ...
  loop_trade(where, e) {
    if (e->item == item) {
      trades_rem_value(&bx[where]->trades, e);
      my_free(e);
    };
  } next_trade;
};
