...
static int
add_new_player(int pl, const char *faction, const char *character, const char *full_name,
	       const char *email, int nation, int start_city)
{
	int who;
...
static void
failed_join(const char *email, const char *reason)
{
  char tmpfile[1024];
  char *cmd;
  FILE *tmp;
  ...
  fprintf(tmp,"\n\n -- The Game Master\n\n");
  fclose(tmp);
  cmd = sout("sendmail -t -odq < %s", tmpfile);
  if (system(cmd))
