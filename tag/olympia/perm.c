...
static int
admit_comp(const void *a, const void *b)
{
	const struct admit * const *ap = a;
	const struct admit * const *bp = b;

	return (*ap)->targ - (*bp)->targ;
}
...
static void
print_att_sup(int who, ilist l, const char *header, int *first)
{
	int i;
	int count = 0;
	char buf[LEN];
