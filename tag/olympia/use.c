...
/* These functions are declared in legacy.h *
 *
 * Forward declarations of commands defined elsewhere are imported via legacy.h.
 * Local helper functions are marked static below.
 */

/* All forward declarations are in legacy.h */
 ...
 */
/* BUGFIX (modernization): update parameters to const void * for qsort */
static int
rep_skill_comp(const void *a, const void *b)
{
	skill_ents_list sa = (skill_ents_list)a;
	skill_ents_list sb = (skill_ents_list)b;
	int pa;		/* parent skill of a */
	int pb;		/* parent skill of b */

#if 1
	if ((*sa)->know != SKILL_know && ((*sb)->know == SKILL_know))
		return -1;

	if ((*sb)->know != SKILL_know && ((*sa)->know == SKILL_know))
		return 1;
#else
	if ((*sa)->know == 0)
	...
	pa = skill_school((*sa)->skill);
	pb = skill_school((*sb)->skill);

	if (pa != pb)
		return pa - pb;

	return (*sa)->skill - (*sb)->skill;
}


/* BUGFIX (modernization): update parameters to const void * for qsort */
static int
flat_skill_comp(const void *a, const void *b)
{
	skill_ents_list sa = (skill_ents_list)a;
	skill_ents_list sb = (skill_ents_list)b;
...
#else
void
list_skill_sup(int who, struct skill_ent *e, const char *prefix)
{
...
being_taught(int who, int sk, int *item, int *teach_bonus)
{
  ...
  /* v_teach and v_study are declared in legacy.h */
