...
static int do_db_url(struct command *c);
static int do_rules_url(struct command *c);
...
static const char *cc_addr = NULL;
...
do_begin(struct command *c)
{
  ...
  if (options.check_balance) {
    ...
    if (result != -1 && result != 0) {
      ...
      err(EAT_ERR, "**                                                     **");
      err(EAT_ERR, "** See http://www.pbm.com/oly/tag/payment.html         **");
      err(EAT_ERR, "**                                                     **");
      err(EAT_ERR, "*********************************************************");
      return FALSE;
    };
 ...
 *
 */
static int do_set(struct command *c)
{
  char *cmd = c->parse[1];
...
eat_banner(void)
{
	...
	out(eat_pl, "X-Loop: orders@tag.pbm.com");
	out(eat_pl, "");
	...
	if (pl) {
	  ...
	  if (result != -1 && result != 0) {
	    ...
	    out(eat_pl, "**                                                     **");
	    out(eat_pl, "** See http://www.pbm.com/oly/tag/payment.html          **");
	    out(eat_pl, "**                                                     **");
	    out(eat_pl, "*********************************************************");
...
v_format(struct command *c)
{
   ...
   *
   */
  for(i=1;i<cstrings_len(c->parse) && c->parse[i];i++) {
    out_alt_who = EAT_OKAY;
