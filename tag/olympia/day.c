...
increment_current_aura(void)
{
	...
	loop_char(who)
	{
	  total = 0;
		...
		if (total) wout(who, "You regenerate %s point%s of mana.",
 ...
 *
 */
/* BUGFIX (modernization): change parameters to const void * for qsort */
static int
followers_comp(const void *a_ptr, const void *b_ptr)
{
  int *a = (int *) a_ptr;
  int *b = (int *) b_ptr;
...
increment_current_piety(void)
{
  ...
  loop_skill(religion) {
	...
	/* BUGFIX (modernization): add missing argument */
	add_piety(who, pow(2, staff_bonus), 0);
      };
    } next_char;
    /*
...
unit_maint_cost(int who, int towho)
{
	...
	if (food && num) {
	  ...
	  if (towho) wout(towho,"Foraging reduces maintenance costs by %s.",
			  gold_s(food));
	  cost -= food;
	};
	...
	if (a = best_artifact(who, ART_MAINTENANCE, 0, 0)) {
	  ...
	  if (food > 0) {
	    if (towho) wout(towho,"Artifact reduces maintenance costs by %s.",
			    gold_s(food));
	    cost -= food;
	  };
	};
...
update_bounties(void)
{
  ...
  loop_city(where) {
	...
	region(where) == cloud_region) continue;
    count = 0;
    loop_trade(where, t) {
      if (t->kind == CONSUME &&
	  ...
	  if (count > 3) {
	    ...
	    trades_rem_value(&bx[where]->trades, t);
	  };
      };
    } next_trade;
    /*
...
change_faery_hills(void)
{
  ...
  loop_subkind(sub_faery_hill, i) {
    ...
    if (exit_views_len(l) > 0 && rnd(1,2) == 1) {
      close_faery_hill(i);
      closed++;
    };
