...
static fights_list defense_side;     /* The side "defending" */
 ...
 *
 */
/* BUGFIX (modernization): updated parameters to const void * for qsort */
int combat_comp(const void *a_ptr, const void *b_ptr)
{
  item_ents_list a = (item_ents_list)a_ptr;
  item_ents_list b = (item_ents_list)b_ptr;
...
static void print_special_banner(void)
{
...
fort_covers(int n)
{
	...
	case sub_guild:			return 50;
	case sub_orc_stronghold:
	case sub_orc_stronghold_notdone: return 100;
	case sub_ship:
	  /*
...
static void
add_fighters(fights_list *l, int who, int ally, int inside, int defender)
{
  struct item_ent *e;
...
static void
add_fight_stack(fights_list *l, int who, int ally, int defender)
{
	int i;
	int inside = FALSE;
 ...
 */
static void
look_for_allies(fights_list *l, int where, int def1, int def2, int
		attacker, int defender, fights_list l_a)
{
  int i;
  ilist tmp;
...
static fights_list
construct_guard_fight_list(int target, int attacker, fights_list
			   l_a, int defender)
{
	fights_list l = NULL;
	int i;
...
static int
special_total_combat_sum(fights_list l)
{
	int i;
	int sum = 0;
 ...
 */
static int
contains_mu_or_undead(fights_list l)
{
  int i;
 ...
 */
static int
num_defenders(struct fight *f, fights_list enemy, int type)
{
  /*
   ...
   */
  if (type == UNCANNY &&
      (f->kind == FK_noble || !f->behind || !f->missile)) return 0;
 ...
 */
static int
total_defenders(fights_list l, fights_list enemy, int type)
{
  int i, sum = 0;
...
has_magic_combat_attack(int who)
{
  ...
  loop_char_skill(who, e) {
	...
	(skill_flags(e->skill) & COMBAT_SKILL)) num_attacks++;
  } next_char_skill;
 ...
 */
static struct fight *
toughest_defender(fights_list l, fights_list enemy, int type)
{
  int i;
 ...
 */
static struct fight *
find_defender(fights_list l, int man, fights_list enemy, int type)
{
  int i;
...
static int
real_resolve_hit(fights_list l_a, struct fight *f,
		 fights_list l_b, struct fight *g, int man, int type,
		 int attack, int defense);
static int
resolve_hit(fights_list l_a, struct fight *f,
	    fights_list l_b, struct fight *g, int man, int type);
 ...
 */
static int
do_magic_combat_attack(fights_list l_a, struct fight *f, fights_list l_b, int skill)
{
  struct fight *g;
  ...
  switch(skill) {
    ...
    case sk_fireball:
	  ...
	  && total_defenders(l_a, l_b, MELEE) > 10) return 0;
    ...
    case sk_raise_soldiers:
      if (!charge_aura(f->unit, skill_piety(skill))) return 0;
      for (i=0;i<fights_len(l_a) && max_raise > 0;i++)
 ...
 */
static int
do_magic_combat_attacks(fights_list l_a, struct fight *f, fights_list l_b)
{
  int sum = 0;
 ...
 */
static int
do_fiery_attack(fights_list l_a, struct fight *f, fights_list l_b)
{
  struct item_ent *e;
  struct fight *g;
 ...
 */
static int
num_attackers(struct fight *f, fights_list enemy, int type)
{
  /*
   *  Forts never attack
   *
 ...
 */
static int
total_attackers(fights_list l, fights_list enemy, int type)
{
  int i, sum = 0;
 ...
 */
static struct fight *
find_attacker(fights_list l, int man, fights_list enemy, int type)
{
  int i;
 ...
 */
static int
count_siege_towers(fights_list l)
{
  int i, num = 0;
...
num_attacks(struct fight *f, int type)
{
  ...
  if (f->kind != FK_noble || 
      count_any_real(f->unit, FALSE, FALSE) > 1) return 1;
 ...
 */
static int
do_attack(int attacker, fights_list l_a, fights_list l_b, int type)
{
  int num_defend;
  struct fight *f;
  struct fight *g;
 ...
 */
static int
attack_round(fights_list l_a, fights_list l_b, int type)
{
...
static int
do_special_attack(fights_list l_a, struct fight *f, fights_list l_b)
{
  int i, sum = 0, man, num_defend, j;
  struct fight *g;
 ...
 */
static int
special_attack(fights_list l_a, int i, fights_list l_b, int type)
{
...
static int
special_attacks(fights_list l_a, fights_list l_b, int type)
{
...
resolve_dead(fights_list l_a)
{
  ...
  for (i = 0; i < fights_len(l_a); i++) {
    ...
    if (g->kind == FK_fort) {
       ...
       *
       */
      /* BUGFIX (modernization): removed excess parameters) */
 ...
 */
static int
run_combat(fights_list *lap, fights_list *lbp)
{
  fights_list l_a = *lap; fights_list l_b = *lbp;
  int num_a, num_b;
...
static int
combat_top(fights_list *lap, fights_list *lbp)
{
        fights_list l_a = *lap; fights_list l_b = *lbp;
	int result;
