 ...
 */
static void
effect_list_print(FILE *fp, char *header, effects_list l)
{
	int i;
	int count = 0;
...
static void
effect_list_scan(char *s, effects_list *l)
{
  struct effect *new;
...
scan_artifact(struct entity_artifact *p, int box_num)
{
  ...
  while (line && iswhite(*line))
	...
	continue;
      ...
      switch (c) {
	case 'ty':	p->type = atoi(t);			break;
	case 'p1':	p->param1 = atoi(t);			break;
	case 'p2':	p->param2 = atoi(t);			break;
	case 'us':	p->uses = atoi(t);			break;
	case 0:
	default:
...
accept_scan(char *s, struct entity_char *pp)
{
  ...
  accept_ents_append(&(pp->accept), p);
}
...
scan_char(struct entity_char *p, int box_num)
{
	...
	while (line && iswhite(*line))
	{
		...
		case 'pb':	p->personal_break_point = atoi(t);break;
		...
		case 'gl':	p->guild = convert_skill(atoi(t));	break;
		case 'pa':	p->pay = atoi(t);		break;
		case 'pi':	p->religion.priest = atoi(t);	break;
		case 'pt':	p->religion.piety = atoi(t);	break;
...
scan_loc(struct entity_loc *p, int box_num)
{
	...
	while (line && iswhite(*line))
	{
		...
		case 'tr':	p->tax_rate = atoi(t);		break;
		case 'fw':	p->control.weight = atoi(t);	break;
		case 'fm':	p->control.men = atoi(t);		break;
		case 'fn':	p->control.nobles = atoi(t);		break;
		case 'cd':	p->control.closed = atoi(t);		break;
...
scan_ship(struct entity_ship *p, int box_num)
{
  ...
  while (line && iswhite(*line)) {
    ...
    switch (c) {
    case 'hu':	p->hulls = atoi(t);			break;
    case 'fo':	p->forts = atoi(t);			break;
    case 'sa':	p->sails = atoi(t);			break;
    case 'po':	p->ports = atoi(t);			break;
    case 'ke':	p->keels = atoi(t);			break;
...
scan_subloc(struct entity_subloc *p, int box_num)
{
  ...
  while (line && iswhite(*line))
	...
	continue;
      ...
      switch (c)
	{
	case 'tm':      p->tax_market = atoi(t);                break;
	case 'hp':	p->hp = atoi(t);			break;
	case 'mt':	p->moat = atoi(t);			break;
...
/*
	...
	*/
	case 'gl':	p->guild = convert_skill(atoi(t));	break;
	case 'es':	p->entrance_size = atoi(t);		break;
...
scan_item(struct entity_item *p, int box_num)
{
	...
	while (line && iswhite(*line))
	{
		...
		case 'tg':	p->trade_good = atoi(t);	break;
		...
		case 'ug':	p->ungiveable = atoi(t);	break;
		case 'wi':	p->wild = atoi(t);	break;
		case 'mt':	p->maintenance = atoi(t);	break;
		case 'sp':	p->npc_split = atoi(t);	break;
		case 'ap':      p->animal_part = atoi(t); break;
...
scan_item_magic(struct item_magic *p, int box_num)
{
	...
	while (line && iswhite(*line))
	{
		...
		case 'rl':	p->religion = atoi(t);			break;
...
scan_player(struct entity_player *p, int box_num)
{
	...
	while (line && iswhite(*line))
	{
		...
		case 'rp':	p->rules_path = str_save(t);	break;
		case 'fp': 
		case 'db':	p->db_path = str_save(t);	break;
		...
		case 'ma':	p->magic = atoi(t);		break;
		case 'js':	p->jump_start = atoi(t);	break;
...
static struct entity_religion_skill *scan_religion(int box_num)
{
  ...
  while (line && iswhite(*line))
	...
	break;
      ...
      switch (c) {
      case 'na':	new->name = str_save(t);	 break;
      case 'st':	new->strength = atoi(t);	 break;
      case 'wk':	new->weakness = atoi(t);	 break;
      case 'pl':        new->plant = atoi(t);		 break;
      case 'an':        new->animal = atoi(t);		 break;
      case 'tr':        new->terrain = atoi(t);	 	 break;
      case 'hp':        new->high_priest = atoi(t);	 break;
      case 'b0':        new->bishops[0] = atoi(t);	 break;
      case 'b1':        new->bishops[1] = atoi(t);	 break;
      };
      advance;
    };
  return new;
...
print_skill(FILE *fp, struct entity_skill *p)
{
  int i;
	...
	for(i=0;i<MAX_FLAGS;i++) {
	  if (p->flags & (1<<i)) {
	    switch (1<<i) {
	    case IS_POLLED: fprintf(fp," pl 1\n");
	      break;
	    case REQ_HOLY_SYMBOL: fprintf(fp," hs 1\n");
	      break;
	    case REQ_HOLY_PLANT: fprintf(fp," hp 1\n");
	      break;
	    case COMBAT_SKILL: fprintf(fp," cs 1\n");
	      break;
	    };
	  };
	};
...
scan_skill(struct entity_skill *p, int box_num)
{
	...
	while (line && iswhite(*line))
	{
		...
		case 'tu':	p->time_to_use = atoi(t);	 break;
		...
		case 'pt':	p->piety = atoi(t);		 break;
		case 'pc':	p->practice_cost = atoi(t);	 break;
		case 'tp':	p->practice_time = atoi(t);	 break;
		case 'pp':	p->practice_prog = atoi(t);	 break;
...
scan_command(struct command *p, int box_num)
{
	...
	while (line && iswhite(*line))
	{
		...
		case 'sw':	p->second_wait = atoi(t);	break;
...
scan_nation2(struct entity_nation *n, int num)
{
  ...
  while (line && iswhite(*line)) {
    ...
    switch (c) {
    case 'na':	n->name = str_save(t); break;
    ...
    case 'ne':  n->neutral = atoi(t); break;
    case 'ps':
 ...
 */
	...
	while (line && *line)
	{
		...
		case 'AR':	scan_artifact(p_item_artifact(n), n);	break;
		case 'CO':	scan_command(p_command(n), n);		break;
		case 'SP':	scan_ship(p_ship(n), n);		break;
		case 'NA':	scan_nation2(p_nation(n), n);		break;
