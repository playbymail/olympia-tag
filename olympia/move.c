...
calc_entrance_fee(struct loc_control_ent *control, struct command *c, int ruler)
{
  ...
  if (control->weight) {
    ...
    if (w_cost) wout(c->who, "Entrance fee is %s on %d weight.", gold_s(w_cost),
		     w.land_weight);
  }
  ...
  if (control->men) {
    ...
    if (m_cost) wout(c->who, "Entrance fee is %s on %d men/beasts.",
		     gold_s(m_cost),
  ...
  if (control->nobles) {
    ...
    if (n_cost) wout(c->who, "Entrance fee is %s on %d nobles.",
		     gold_s(n_cost),
...
#define MAX(a,b) ((a) > (b) ? (a) : (b))
  case sub_ship:
    ...
    if (sails < sp->hulls/2) {
       ...
       *
       */
      sail_time -= (((sails - 2*sp->hulls)/sp->hulls)) * 2;
    } else if (sails < 2 * sp->hulls) {
      sail_time += 2;
    };
