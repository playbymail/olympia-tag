...
static void call_init_routines(void);
static int make_report(int format, char *fnam, char *report, int pl);
static void mail_reports(void);
static void output_html_map(int pl);
static void output_html_rep(int pl);
static int preprocess(char *in, char *out, char *args);
static void set_html_pass(int pl);
static void setup_html_all(void);
static void setup_html_dir(int pl);
static void write_email(void);
static void write_factions(void);
static void write_faction_sup(int who_for, int target, FILE *fp);
static void write_forwards(void);
static void write_forward_sup(int who_for, int target, FILE *fp);
static void write_nations_lists(void);
static void write_player_list(void);
static void write_totimes(void);
...
send_rep(int pl, int turn)
{
   ...
   *
   */
  cmd = sout("sendmail -odq %s < %s/Times", p->email, libdir);
  ret = system(cmd);
  if (ret) 
	fprintf(stderr, "send_rep: mail to %s failed: %s\n",
		p->email, cmd);
...
setup_html_all(void)
{
  ...
  loop_player(pl) {
    if (!is_real_npc(pl)) {
      ...
      output_html_rep(pl);
      output_html_map(pl);
      fprintf(fp,"<TR><TD><A HREF=\"http://www.pbm.com/oly/tag/reports"
