 ...
 *
 */
static void add_trade(int where, int type)
{
  int i, found, new_tg;
...
update_market(int where)
{
  ...
  loop_trade(where, t) {
	...
	t->counter > options.market_age) {
      ...
      trades_rem_value(&bx[where]->trades, new);
      continue;
    };
 ...
 *
 */
static struct trade *traded_here(int where, int good)
{
  struct trade *t;
 ...
 *
 */
static struct trade *produced_here(int where, int good)
{
  struct trade *t;
