...
add_build(int what, int t, int bm, int er, int eg)
{
   ...
   *
   */
  entity_builds_append(&rp_subloc(what)->builds, new);
  return 1;
};
...
delete_build(int what, int type)
{
  ...
  for (i = entity_builds_len(e) - 1; i >= 0; i--) {
  	// todo: is this a bug? the return is indented like it should be part of the if.
    if (e[i]->type == type) entity_builds_delete(&rp_subloc(what)->builds, i);
      return;
  };
};
...
struct entity_build *get_build(int what, int t)
{
   ...
   *
   */
  for(i=0;i < entity_builds_len(e);i++)
...
struct build_ent
{
	const char *what;				/* what are we building? */
