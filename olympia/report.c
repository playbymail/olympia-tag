...
static int
output_order_comp(const void *a_ptr, const void *b_ptr)
{
	int *a = (int *)a_ptr;
	int *b = (int *)b_ptr;
...
/* BUGFIX (modernization): update parameters to const void * for qsort */
int
inv_item_comp(const void *a_ptr, const void *b_ptr)
{
...
list_accepts(int who, int num)
{
  ...
  for (i = 0; i < accept_ents_len(accept); i++) {
