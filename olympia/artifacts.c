 ...
 * Added missing `item_used` parameter. Seems likely that at one point
 * it returned the artifact that generated the bonus. Did not wire it
 * in
 */
int
combat_artifact_bonus(int who, int part, int *item_used)
{
  struct item_ent *e;
  int best = 0;
 ...
 */
static void
describe_combat_artifact(int who, int target, const char *header)
{
  int first = 1, i, val;
  const char *buf = 0; const char *total = 0;
 ...
 */
int
artifact_identify(const char *header, struct command *c)
{
  int target = c->a;
  char *f;
  int type;
